...
export default function EmbedConfig() {
    const empty = { enabled: true, title: '', description: '', provider: "", author: "", color: '' }
    const [firstEmbed, setFirstEmbed] = useState<EmbedConfig>(empty)
    const [embed, setEmbed] = useState<EmbedConfig>(empty)
    ...
    function setMasterEmbed(embed: EmbedConfig) {
        setFirstEmbed(embed)
        setEmbed(embed)
    }
    ...
    return <div className='grid grid-cols-2 gap-10 h-full'>
        <div className='bg-primary p-3 rounded-lg shadow-lg  col-span-2 md:col-span-1 relative pb-24'>
            ...
            <div className='grid grid-cols-2 gap-3'>
                ...
                <div>
                    ...
                    <AutoCompleteInput maxChars={256} value={embed.provider} onChange={(v) => updateEmbed("provider", v)} type='input' options={AutocompleteOptions} />
                </div>
                <div>
                    <p className='text-xs opacity-40 pl-1'>Author</p>
                    <AutoCompleteInput maxChars={256} value={embed.author} onChange={(v) => updateEmbed("author", v)} type='input' options={AutocompleteOptions} />
                </div>
                <div>
                    <p className='text-xs opacity-40 pl-1'>Title</p>
                    <AutoCompleteInput maxChars={256} value={embed.title} onChange={(v) => updateEmbed("title", v)} type='input' options={AutocompleteOptions} />
                </div>
                <div>
                    <p className='text-xs opacity-40 pl-1'>Color</p>
                    <div className='grid grid-cols-2 gap-2'>
                        <input className='h-full w-full bg-primary rounded-lg' type='color' value={embed.color} onChange={(e) => updateEmbed("color", e.target.value)} />
                        <AutoCompleteInput value={embed.color} onChange={(v) => updateEmbed("color", v)} type='input' />
                    </div>
                </div>
                <div className='col-span-2'>
                    <p className='text-xs opacity-40 pl-1'>Description</p>
                    <AutoCompleteInput maxChars={4096} value={embed.description} onChange={(v) => updateEmbed("description", v)} type='textarea' options={AutocompleteOptions} />
                </div>
            </div>
            <AnimatePresence>
                {openSaveModal && <motion.div
