		...
		<div className='flex flex-col gap-3'>
			<div className='flex gap-3 flex-col md:flex-row'>
				<div className='bg-primary p-3 rounded-lg shadow-lg w-full md:w-1/2'>
					...
					<p className='text-xl font-bold opacity-50'>Manage all your domains and check the status of them</p>
				</div>
				<div className='bg-primary p-3 rounded-lg shadow-lg w-full md:w-1/2'>
					<p className='text-4xl font-bold'>Active Domain</p>
					<p className='text-xl font-bold opacity-75'>
                        You are currently using <span className='font-bold text-orange-500'>thykka.xyz</span> as your domain.
					</p>
					<p className='text-sm opacity-40'>Click on any of the play buttons down below to change your domain</p>
				</div>
			</div>
			<div className='bg-primary p-3 rounded-lg shadow-lg w-full flex flex-col gap-3'>
				<div className='bg-secondary rounded-md bg-opacity-75 p-3 text-lg font-bold text-center overflow-scroll'>
					...
					{domains.map((domain, i) => (
						<div key={i} className='bg-secondary even:bg-primary first:rounded-t-md last:rounded-b-md bg-opacity-75 flex px-3 text-sm font-bold text-center h-10'>
							<div className='w-1/6 min-w-[10rem]'>
								<p>{domain.domain}</p>
							</div>
							<div className='w-1/5 min-w-[10rem]'>
								<p style={{color: cleanText[domain.status].color}}>{cleanText[domain.status].text}</p>
							</div>
							<div className='w-1/5 min-w-[10rem]'>
