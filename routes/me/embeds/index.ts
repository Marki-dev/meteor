...
router.post('/', WebAuthHandler, async (req, res) => {
  const { enabled, name, title, description, provider, author, color } =
    req.body;
  ...
  if (title && title.length > 256)
    return res
      .status(400)
      .json({ error: 'Title cannot exceed 256 characters' });
  if (description && description.length > 2048)
    return res
      .status(400)
      .json({ error: 'Description cannot exceed 2048 characters' });
  if (provider && provider.length > 256)
    return res
      .status(400)
      .json({ error: 'Provider name cannot exceed 256 characters' });
  if (author && author.length > 256)
    return res
      .status(400)
      .json({ error: 'Author name cannot exceed 256 characters' });
